...
def change(d, c): # d -> 기존 방향, c -> times = {X : C} 구조에서 C(방향)
...
def start():
    ...
    while True:
        ...
        # 갱신된 x, y가 N x N 배열 범위이고 아직 방문하지 않았을 때
        # (이미 방문한 곳이면 뱀 몸뚱이가 한 점에서 만난 것)
        if (0 <= y < N) and (0 <= x < N) and (arr[y][x] != 2):
            ...
            visited.append([y, x]) # 갱신된 [y, x]를 뱀 머리로 추가
            if time in times.keys(): # 현재 시간이 뱀 방향정보에 있을 때.
