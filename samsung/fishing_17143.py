        ...
        for j in range(C):
            if g[i][j] != 0:
                ...
                while s > 0: # 상어가 한 번 이동할 때 가는 거리(속도)
                    ...
                    else: # 격자판 범위 초과 -> x(R 행), y(C 열) 좌표 갱신 + d(방향 갱신)
                        x -= dx[d] # 기존 dx[d]에 (-)로 x, y 좌표 변환
                        y -= dy[d] # dx dy로 상하좌우 활용
                        if d == 0: d = 1 # (0 : 위) -> (1 : 아래)
                        elif d == 1: d = 0 # (1 : 아래) -> (1 : 위)
                        elif d == 2: d = 3 # (2 : 우) -> (1 : 좌)
                        elif d == 3: d = 2 # (3 : 좌) -> (1 : 우)
                        # 격자판 벽에 턴하고 방향 반대 전환

                if temp[x][y] == 0: # 해당 자리에 상어 없으면 예정된 상어 끌어오기
                    temp[x][y] = [g[i][j][0], d, z] # while문에서 s가 계속 차감되어 0이라 초기 s인 g[i][j][0]
